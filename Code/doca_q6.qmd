...
An alternative method is to turn our estimates into predicted probabilities and then calculate the average effect of a one-unit change in an independent variable on the change in *probability* of the outcome of interest (a marginal effect). The challenge here is that the effect of a one-unit change is non-linear with respect to the probability of the outcome variable:

```{r}

# making some hypothetical data (the log_participants value
# is unrealistically high here, but this is just an illustration)
newdata<-expand.grid(post_1969 = "Before 1969", 
                    propdam = c("Yes", "No"), 
                    log_participants=seq(0, 100, by=1))
predictions<-predict(model, newdata= newdata, type='response')

ggplot(newdata, aes(x=log_participants, y=predictions, color=propdam)) + 
  geom_line() +
  theme_bw() +
  labs(x= "Participants", y="Pr(police force)",
       color ='property damage?'
       ) 
```

So, our marginal effect is really "the marginal effect of a one unit change in X at some specified value of the independent variables", and we'll often have to make an arbitrary decision to report a marginal effect at a specific point. In this situation, a good default strategy is just to do the following:

1.  Get a prediction for every case at its current observed value and average those predictions.
2.  Increase the value of a single IV by one unit for all cases and calculate a new prediction and then average that.
3.  Calculate the average difference between the observed case and the new average prediction.

This would give us an average marginal effect of a one unit increase in an IV for every case.

We could do this manually, but the [marginaleffects package](https://marginaleffects.com/) has some commands that will automatically perform this calculation for a given model:
...
We can also use the `plot_predictions` function to get overall predicted probabilities for these cases:

```{r}

plot_predictions(model, condition='post_1969')

```

```{r}

plot_predictions(model, condition='log_participants')
...
plot_predictions(model, condition='propdam')
